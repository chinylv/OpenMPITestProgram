...
int main(int argc, char *argv[])
{
  ...
  int reorder = 1; // so that the rank would change in the new communicator

  dim_sizes[0] = size;
  dim_sizes[1] = numtasks/size;

  // create a new communicator, make one array to be multi-dimensional array
  // combine this feature with matlab
  // the new comm is dim_sizes-dimensional, and every dimension has wrap_around[0...dim_size-1] processors respectively
  MPI_Cart_create(MPI_COMM_WORLD, 2, dim_sizes, wrap_around, reorder, &new_comm);

  int coords[2];
  int my_grid_rank;
  MPI_Comm_rank(new_comm, &my_grid_rank);
  // the process coords in new_comm
